  ...
  if (data.length === 0) {
    return (
      <motion.div 
        ...
        className="p-12 text-center text-ui-muted italic bg-ui-card rounded-[2.5rem] border border-ui-border text-[16px]"
      >
        ไม่พบข้อมูลที่ค้นหา 🔍
      </motion.div>
    );
  ...
  return (
    <div className="w-full flex-1 flex flex-col bg-transparent overflow-hidden">
      <div className="flex-1 overflow-auto custom-scrollbar">
        <table className={`text-left text-[15px] border-separate border-spacing-0 ${activeTab === "address" ? "min-w-[2800px]" : "min-w-full"}`}>
          <thead className="sticky top-0 z-20">
            <tr className="bg-ui-bg text-ui-muted uppercase tracking-wider text-[14px] font-black">
              {activeTab === "customer" && (
                <>
                ...
                return (
                  ...
                  >
                    ...
                    {activeTab === "customer" && isCustomer(item) && (
                      <>
                        <td className={`${cellPadding} whitespace-nowrap font-mono text-brand-primary font-bold w-[120px] ${stickyLeftCell}`}>
                          {item.customer_code}
                        </td>
                        <td className={`${cellPadding} whitespace-nowrap font-mono text-[14px]`}>
                          {item.ean_location_code ?? "-"}
                        </td>
                        <td className={`${cellPadding} whitespace-nowrap font-black`}>
                          {item.company_name ?? "-"}
                        </td>
                        <td className={`${cellPadding} text-right whitespace-nowrap text-ui-muted font-black`}>
                          {item.short_name ?? "-"}
                        </td>
                      </>
                    )}
                    ...
                    {activeTab === "product" && isProduct(item) && (
                      <>
                        <td className={`${cellPadding} whitespace-nowrap font-mono text-brand-secondary font-bold w-[150px] ${stickyLeftCell}`}>
                          {item.ean_product_code}
                        </td>
                        <td className={`${cellPadding} whitespace-nowrap font-mono font-bold`}>
                    ...
                    {activeTab === "address" && isAddress(item) && (
                      <>
                        ...
                        <td className={`${cellPadding} whitespace-nowrap font-black text-ui-muted`}>
                    ...
                    <td className={`${cellPadding} ${stickyRightCell}`}>
                      <div className="flex justify-center gap-2">
                        <button onClick={() => onView(item)} className="px-3 py-1.5 rounded-xl bg-blue-500/10 text-blue-600 hover:bg-blue-500 hover:text-white transition-all text-[13px] font-black border border-blue-500/20">
                          View
                        </button>
                        <button onClick={() => onEdit(item)} className="px-3 py-1.5 rounded-xl bg-brand-secondary/10 text-brand-secondary hover:bg-brand-secondary hover:text-white transition-all text-[13px] font-black border border-brand-secondary/20">
                          Edit
                        </button>
                        {isAdmin && (
                          <button onClick={() => onDelete(rawId)} className="px-3 py-1.5 rounded-xl bg-status-error/10 text-status-error hover:bg-status-error hover:text-white transition-all text-[13px] font-black border border-status-error/20">
                            Del
                          </button>
                        )}
                      </div>
      ...
      <div className="bg-ui-bg/50 border-t border-ui-border p-4 flex items-center justify-between">
        <div className="text-ui-muted text-[13px] font-black uppercase tracking-widest pl-4">
          PAGE <span className="text-brand-primary">{currentPage}</span> / {totalPages}
        </div>
        <div className="flex gap-2 pr-4">
          <button onClick={onPrevPage} disabled={currentPage === 1} className="px-4 py-2 rounded-2xl bg-ui-card text-ui-text border border-ui-border hover:border-brand-primary disabled:opacity-30 transition-all font-black text-[13px] shadow-sm active:scale-95">
            PREV
          </button>
          <button onClick={onNextPage} disabled={currentPage === totalPages} className="px-4 py-2 rounded-2xl bg-ui-card text-ui-text border border-ui-border hover:border-brand-primary disabled:opacity-30 transition-all font-black text-[13px] shadow-sm active:scale-95">
            NEXT
          </button>
        </div>
      </div>
